...
/// Robust natural context crop:
/// - 1.35x padding to capture chin, hairline, and facial boundary without zooming in too tight
/// - Centers slightly upward (bh * 0.43) to naturally position eyes near y ≈ 51
fn natural_context_crop(img: &DynamicImage, d: &Detection) -> DynamicImage {
    let bw = d.x2 - d.x1;
    let bh = d.y2 - d.y1;
...
fn embed_raw_chip(session: &mut Session, img: &DynamicImage) -> Vec<f32> {
    let rgb = img.to_rgb8();
...
/// Generates a stabilized embedding by averaging the crop with its horizontal flip
fn extract_stabilized_embedding(session: &mut Session, chip: &DynamicImage) -> Vec<f32> {
    let emb_orig = embed_raw_chip(session, chip);
    let emb_flip = embed_raw_chip(session, &chip.fliph());

    let mut combined = vec![0.0f32; 512];
    for k in 0..512 {
        combined[k] = emb_orig[k] + emb_flip[k];
    }

    let norm: f32 = combined.iter().map(|v| v * v).sum::<f32>().sqrt().max(1e-6);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, path) in &photos {
        ...
        for (idx, det) in detections.iter().enumerate() {
            let chip = natural_context_crop(&img, det);
            let embedding = extract_stabilized_embedding(&mut rec_sess, &chip);

            faces.push(FaceEntity {
    ...
    println!("\n=== NATURAL CONTEXT + FLIP-STABILIZED w600k_r50 ===");
    println!("Threshold: 0.35 (Match >= 0.35, Different < 0.35)\n");

    let match_thresh = 0.35f32;
    ...
    for i in 0..faces.len() {
        for j in (i + 1)..faces.len() {
            ...
            println!(
                "{:<10} vs {:<10} -> Similarity: {:<6.4}  [{}]",
                a.label, b.label, sim,
                if is_match { "MATCH" } else { "Different" }
            );

            comparisons.push((a.chip.clone(), b.chip.clone(), sim, is_match));
        }
    }
    ...
    if !comparisons.is_empty() {
        ...
        let out_file = PathBuf::from("diag_output/final_clean_matrix.jpg");
        canvas.save(&out_file)?;
        println!("\nSaved finalized visual grid to: {}", out_file.display());
    }
