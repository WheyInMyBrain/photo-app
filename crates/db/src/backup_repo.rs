...
impl BackupRepo {
    /// Fetches all items to sync with B2:
    ...
    pub async fn fetch_pending_sync_items(
        ...
        // 1. PENDING UPLOADS: Active items only (skip items sitting in trash)
        ...
        // 2. PENDING REMOTE DELETIONS: Backed-up items where local asset was permanently deleted
        let deletion_rows = sqlx::query(
            r#"
            SELECT 
                b.asset_id,
                b.user_id,
                b.remote_path
            FROM asset_backups b
        ...
        for r in deletion_rows {
            candidates.push(BackupCandidate {
                ...
                username: String::new(),
                disk_rel_path: String::new(),
                suggested_remote_path: r.get("remote_path"),
                sha256: String::new(),
                is_deleted: true,
            });
        }
