...
impl IngestionRepo {
    /// Commits the entire post-processing payload atomically.
    pub async fn commit_processed_asset(
        ...
        if !payload.new_persons.is_empty() {
            ...
            qb.push(" ON CONFLICT(id) DO NOTHING"); // Prevent collision if multiple faces spawn the same cluster
        ...
        if !payload.detected_faces.is_empty() {
            ...
            qb.push_values(payload.detected_faces, |mut b, face| {
                ...
                // Map empty string to NULL so SQLite foreign key doesn't reject it
