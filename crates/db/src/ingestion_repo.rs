...
impl IngestionRepo {
    ...
    pub async fn commit_processed_asset(
        ...
        if !payload.detected_faces.is_empty() {
            ...
            let mut valid_person_set: std::collections::HashSet<String> =
                std::collections::HashSet::new();

            if !referenced_person_ids.is_empty() {
                let mut check_qb: QueryBuilder<Sqlite> =
                    QueryBuilder::new("SELECT id FROM persons WHERE user_id = ");
                check_qb.push_bind(&payload.asset.user_id);
            ...
            qb.push_values(payload.detected_faces, |mut b, face| {
                ...
                let person_id_opt = if !face.person_id.trim().is_empty()
                    && valid_person_set.contains(face.person_id.trim())
                {
                    Some(face.person_id)
                } else {
                    None
    ...
    pub async fn commit_ai_metadata(
        ...
        if let Some(emb) = payload.clip_embedding {
            sqlx::query(
                "UPDATE assets SET clip_embedding = ?, clip_processed = 1 WHERE id = ? AND user_id = ?",
            )
            .bind(emb)
            .bind(&payload.asset_id)
            .bind(&payload.user_id)
        ...
        if !payload.detected_faces.is_empty() {
            ...
            sqlx::query("UPDATE assets SET face_processed = 1 WHERE id = ? AND user_id = ?")
                .bind(&payload.asset_id)
                .bind(&payload.user_id)
                .execute(&mut *tx)
                .await?;
        ...
        if !payload.tags.is_empty() {
            ...
            sqlx::query("UPDATE assets SET tags_processed = 1 WHERE id = ? AND user_id = ?")
                .bind(&payload.asset_id)
                .bind(&payload.user_id)
                .execute(&mut *tx)
                .await?;
